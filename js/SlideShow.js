...
SlideShow.prototype.events = function(){
	...
	*/
	...
	this.view.addEventListener('touchend', function(e){
		...
		//console.log( deltaX, deltaY, timeDelta );
		if(  Math.abs(deltaY) > Math.abs(deltaX) // Y movement was greater than X movement
			|| Math.abs(deltaX) < SlideShow.MIN_DELTA_FOR_SWIPE // Not enough movement
			|| timeDelta > SlideShow.MAX_TIME_FOR_SWIPE) {  // Took too long, is probably not a swipe
			return; 
		}
		//console.log("Checking for swipe: success!");
...
SlideShow.prototype.buildIndicator = function(){
	var self = this;
...
SlideShow.prototype.startLoop = function(){
	...
	function loop(){
		if(self.playing){
            tracking.handleAutoRotate(self.view, self.active_index);
			self.next();
